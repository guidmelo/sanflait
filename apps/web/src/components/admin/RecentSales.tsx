...
const PRICE_COLOR = {
  CONVERTIDO: 'text-accent-teal',
  EM_NEGOCIACAO: 'text-accent-amber',
  PERDIDO: 'text-accent-red',
};
  ...
  return (
    <div className="space-y-1.5">
      {sales.slice(0, 5).map((s) => (
        <div
          key={s.id}
          className="flex justify-between items-center py-1.5 border-b border-line last:border-b-0 last:pb-0"
        >
          ...
          <div className="text-right flex-shrink-0 ml-3">
            <p className={cn('text-[11px] font-semibold', PRICE_COLOR[s.status])}>
              {formatBRL(s.amount)}
            </p>
            <span className={cn('admin-pill mt-1', STATUS_STYLES[s.status])}>
